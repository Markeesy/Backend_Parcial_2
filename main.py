...
def actualizar_vehiculo(
    id: int, item: schemas.VehiculoUpdate, db: Session = Depends(get_db)
):
...
@app.post(
    "/calles", response_model=schemas.CalleResponse, status_code=status.HTTP_201_CREATED
)
def crear_calle(item: schemas.CalleCreate, db: Session = Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, detail=f"Ocurrio un error al guardar la calle: {str(e)}"
        )
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Ocurrio un error al guardar la calle: {str(e)}"
        )
...
def leer_calles(db: Session = Depends(get_db)):
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Ocurrio un error al leer las calles: {str(e)}"
        )
    return items
...
def leer_calle(id: int, db: Session = Depends(get_db)):
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Ocurrio un error al leer la calle: {str(e)}"
        )
    return item
...
def actualizar_calle(id: int, item: schemas.CalleUpdate, db: Session = Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, detail=f"Ocurrio un error al actualizar la calle: {str(e)}"
        )
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Ocurrio un error al actualizar la calle: {str(e)}"
        )
    return db_item
...
@app.post(
    "/estacionamientos",
    response_model=schemas.EstacionamientoResponse,
    status_code=status.HTTP_201_CREATED,
)
def crear_estacionamiento(
    item: schemas.EstacionamientoCreate, db: Session = Depends(get_db)
):

    # Validamos que los datos de entrada sean validos
    if item.idCalle is None:
        raise HTTPException(status_code=400, detail="Calle es requerida")
    if item.numeroCalle is None:
    ...
    if (
        db.query(models.Estacionamiento)
        .filter(
            models.Estacionamiento.idCalle == item.idCalle,
            models.Estacionamiento.numeroCalle == item.numeroCalle,
        )
        .first()
    ):
        raise HTTPException(status_code=400, detail="Estacionamiento ya existe")

    db_item = models.Estacionamiento(idCalle=item.idCalle, numeroCalle=item.numeroCalle)
    ...
    try:
        ...
        raise HTTPException(
            status_code=400,
            detail=f"Ocurrio un error al guardar el estacionamiento: {str(e)}",
        )
    except Exception as e:
        raise HTTPException(
            status_code=500,
...
def leer_estacionamientos(db: Session = Depends(get_db)):
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Ocurrio un error al leer los estacionamientos: {str(e)}",
        )
    return items
    ...
    try:
        item = (
            ...
            .filter(
                ...
                models.Estacionamiento.numeroCalle == numeroCalle,
            )
            .first()
        )
        if item is None:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Ocurrio un error al leer el estacionamiento: {str(e)}",
        )
...
def eliminar_estacionamiento(
    idCalle: int, numeroCalle: int, db: Session = Depends(get_db)
):
    db_item = (
        db.query(models.Estacionamiento)
        .filter(
    ...
    try:
        ...
        raise HTTPException(
            status_code=400,
            detail=f"Ocurrio un error al eliminar el estacionamiento: {str(e)}",
        )
    except Exception as e:
        raise HTTPException(
            status_code=500,
...
def eliminar_calle(id: int, db: Session = Depends(get_db)):
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Ocurrio un error al eliminar la calle: {str(e)}"
        )
    return item
...
def crear_registro(item: schemas.RegistroCreate, db: Session = Depends(get_db)):
    ...
    if (
        not db.query(models.Estacionamiento)
        .filter(
            models.Estacionamiento.idCalle == db_item.idCalle,
            models.Estacionamiento.numeroCalle == db_item.numeroCalle,
        )
        .first()
    ):
        raise HTTPException(status_code=404, detail="Estacionamiento no encontrado")
    ...
    if db_item2:
        raise HTTPException(
            status_code=400,
            detail=f"Estacionamiento ocupado por vehiculo {db_item2.idVehiculo}",
        )

    # Validamos que el vehiculo no tenga otra registracion en el mismo horario
    db_item_vehiculo = (
        db.query(models.Registro)
        .filter(
            models.Registro.idVehiculo == db_item.idVehiculo,
            models.Registro.fechaHoraEntrada <= fechaHoraSalida,
            models.Registro.fechaHoraSalida >= db_item.fechaHoraEntrada,
            models.Registro.anulado == False,
        )
    ...
    if db_item_vehiculo:
        raise HTTPException(
            status_code=400,
            detail="El vehiculo ya tiene un registro en ese horario",
        )
...
def leer_registros(db: Session = Depends(get_db)):
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Ocurrio un error al leer los registros: {str(e)}"
        )
...
def leer_estacionamientos_ocupados(
    item: schemas.ConsultaEstacionamientos, db: Session = Depends(get_db)
):
    ...
    if idCalle is None and numeroCalle is not None:
        raise HTTPException(
            status_code=400,
            detail="Si ingresa numero de calle debe ingresar tambien la calle",
        )
    ...
    if numeroCalle is not None and idCalle is not None:
        if (
            not db.query(models.Estacionamiento)
            .filter(
                models.Estacionamiento.idCalle == idCalle,
            ...
            raise HTTPException(
                status_code=404, detail="El estacionamiento consultado no existe"
            )

    if fechaHoraConsulta is None:
    ...
    filtrosBusqueda = [
        ...
        models.Registro.anulado is False,
    ]

    if idCalle is not None:
        filtrosBusqueda.append(models.Registro.idCalle == idCalle)
    if numeroCalle is not None:
        filtrosBusqueda.append(models.Registro.numeroCalle == numeroCalle)

    db_item = db.query(models.Registro).filter(*filtrosBusqueda).all()
...
@app.get(
    "/estacionamientos_ocupados_por_rango",
    response_model=List[schemas.EstacionamientoOcupadoRangoResponse],
)
def leer_estacionamientos_ocupados_por_rango(
    item: schemas.ConsultaEstacionamientosPorRango, db: Session = Depends(get_db)
):
    ...
    calles_bd = (
        db.query(models.Calle).filter(models.Calle.idCalle.in_(calles_ids)).all()
    )
...
@app.get(
    ...
    response_model=List[schemas.EstacionamientoDisponibleResponse],
)
def leer_estacionamientos_disponibles(
    item: schemas.ConsultaEstacionamientos, db: Session = Depends(get_db)
):
    idCalle = item.idCalle
    numeroCalle = item.numeroCalle
    ...
    disponibles = [
        est
        for est in todos_estacionamientos
        if (est.idCalle, est.numeroCalle) not in set_ocupados
    ]

    if not disponibles:
        raise HTTPException(
            status_code=404, detail="No hay estacionamientos disponibles"
        )
...
@app.get(
    "/estacionamientos_disponibles_por_rango",
    response_model=List[schemas.EstacionamientoDisponibleResponse],
)
def leer_estacionamientos_disponibles_por_rango(
    item: schemas.ConsultaEstacionamientosPorRango, db: Session = Depends(get_db)
):
    idCalle = item.idCalle
    numeroCalle = item.numeroCalle
...
def crear_cliente(item: schemas.ClienteCreate, db: Session = Depends(get_db)):
    db_item = models.Cliente(nombreCliente=item.nombreCliente)
    if not db_item.nombreCliente:
        raise HTTPException(status_code=400, detail="Nombre de cliente es requerido")
    try:
        db.add(db_item)
        db.commit()
        db.refresh(db_item)
        ...
        raise HTTPException(
            status_code=500, detail=f"Ocurrio un error al guardar el cliente: {str(e)}"
        )
...
def leer_clientes(db: Session = Depends(get_db)):
    try:
        items = db.query(models.Cliente).all()
        items.sort(key=lambda x: x.idCliente)
        return items
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Ocurrio un error al leer los clientes: {str(e)}"
        )


@app.get("/clientes/{id}", response_model=schemas.ClienteResponse)
def leer_cliente(id: int, db: Session = Depends(get_db)):
    try:
        item = db.query(models.Cliente).filter(models.Cliente.idCliente == id).first()
        if item is None:
        ...
        raise HTTPException(
            status_code=500, detail=f"Ocurrio un error al leer el cliente: {str(e)}"
        )


@app.put("/clientes/{id}", response_model=schemas.ClienteResponse)
def actualizar_cliente(
    id: int, item: schemas.ClienteUpdate, db: Session = Depends(get_db)
):
    db_item = db.query(models.Cliente).filter(models.Cliente.idCliente == id).first()
    if db_item is None:
        raise HTTPException(status_code=404, detail="Cliente no encontrado")
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Ocurrio un error al actualizar el cliente: {str(e)}"
        )


@app.delete("/clientes/{id}", response_model=schemas.ClienteResponse)
def eliminar_cliente(id: int, db: Session = Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Ocurrio un error al eliminar el cliente: {str(e)}"
        )


# CRUD Vehiculos-Clientes (Vinculación)

@app.post(
    "/vehiculos_clientes",
    response_model=schemas.VehiculoClienteResponse,
    status_code=status.HTTP_201_CREATED,
)
def crear_vehiculo_cliente(
    item: schemas.VehiculoClienteCreate, db: Session = Depends(get_db)
):
    # Validamos que el vehiculo exista
    if not db.query(models.Vehiculo).filter(models.Vehiculo.idVehiculo == item.idVehiculo).first():
        raise HTTPException(status_code=404, detail="Vehiculo no encontrado")

    # Validamos que el cliente exista
    if not db.query(models.Cliente).filter(models.Cliente.idCliente == item.idCliente).first():
        raise HTTPException(status_code=404, detail="Cliente no encontrado")

    db_item = models.VehiculoCliente(idVehiculo=item.idVehiculo, idCliente=item.idCliente)
    try:
        db.add(db_item)
        db.commit()
        db.refresh(db_item)
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Ocurrio un error al guardar la vinculación: {str(e)}",
        )


@app.get("/vehiculos_clientes", response_model=List[schemas.VehiculoClienteResponse])
def leer_vehiculos_clientes(db: Session = Depends(get_db)):
    try:
        items = db.query(models.VehiculoCliente).all()
        items.sort(key=lambda x: x.idVehiculoCliente)
        return items
    except Exception as e:
        raise HTTPException(
            status_code=500,
            detail=f"Ocurrio un error al leer las vinculaciones: {str(e)}",
        )


@app.get("/vehiculos_clientes/{id}", response_model=schemas.VehiculoClienteResponse)
def leer_vehiculo_cliente(id: int, db: Session = Depends(get_db)):
    try:
        item = (
            db.query(models.VehiculoCliente)
            .filter(models.VehiculoCliente.idVehiculoCliente == id)
            .first()
        )
        if item is None:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Ocurrio un error al leer la vinculación: {str(e)}",
        )


@app.put("/vehiculos_clientes/{id}", response_model=schemas.VehiculoClienteResponse)
def actualizar_vehiculo_cliente(
    id: int, item: schemas.VehiculoClienteUpdate, db: Session = Depends(get_db)
):
    db_item = (
        db.query(models.VehiculoCliente)
        .filter(models.VehiculoCliente.idVehiculoCliente == id)
    ...
    # Si se actualiza el idVehiculo, validamos que exista
    if item.idVehiculo is not None:
        if not db.query(models.Vehiculo).filter(models.Vehiculo.idVehiculo == item.idVehiculo).first():
            raise HTTPException(status_code=404, detail="Vehiculo no encontrado")

    # Si se actualiza el idCliente, validamos que exista
    if item.idCliente is not None:
        if not db.query(models.Cliente).filter(models.Cliente.idCliente == item.idCliente).first():
            raise HTTPException(status_code=404, detail="Cliente no encontrado")
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Ocurrio un error al actualizar la vinculación: {str(e)}",
        )


@app.delete("/vehiculos_clientes/{id}", response_model=schemas.VehiculoClienteResponse)
def eliminar_vehiculo_cliente(id: int, db: Session = Depends(get_db)):
    item = (
        db.query(models.VehiculoCliente)
        .filter(models.VehiculoCliente.idVehiculoCliente == id)
        .first()
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Ocurrio un error al eliminar la vinculación: {str(e)}",
        )
